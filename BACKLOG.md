...
- [x] **Bibliothèque + file de qualification** : indexeur (`scripts/indexer`) → upsert Postgres (`library_document`), mode IMP (porte démo) + écran de qualification (titre, géographie à 3 niveaux, dimensions, tags), ressources internes qualifiées affichées dans le contexte des dimensions + pointeurs passés à l'analyse. Purge des fichiers disparus faite avec le RAG (jamais sur un scan vide : un partage injoignable ne doit pas effacer les qualifications). *Reste :* auth réelle = P2.
...
- [x] **Référentiel méthodologique du client** (`content/<client>/definitions.json`) : définitions « Populations défavorisées » et « Territoires défavorisés » (fournies par Hugo, 2026-07-08) injectées dans les prompts de couverture et d'avis qualitatif des dimensions concernées (elles priment sur l'acception générique) et affichées dans « Parcourir les critères ». Extensible : ajouter un terme = une entrée JSON (terme, dims, definition).
- [x] **RAG banque interne** (3 tranches, 2026-07-08) : pgvector (`library_chunk`, HNSW) + embeddings locaux (`multilingual-e5-small`, rien ne sort de l'infra, `EMBEDDINGS_PROVIDER` pour en changer) ; indexation dans `pnpm indexer` (extraction vision des PDF, découpage ~1 000 caractères avec repères de page, purge des disparus) ; à l'analyse, top 6 passages par dimension (restreints aux documents que `libForDim` retient) injectés dans les deux prompts avec consigne de citation « titre, p.X », documents mobilisés affichés dans les résultats, échec de recherche signalé. *Restes connus :* `.docx`/`.xlsx` non indexés (signalés), requalification des `dims`/`geo` sans réindexation = OK (filtrage à la requête).
