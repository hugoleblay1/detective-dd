...
1. **Contexte externe, résumé à l'écran.** Par dimension × géographie, quelques indicateurs de base directement affichés (ex. biodiversité au Kenya : aires protégées majeures, indicateurs clés) — pré-extraits côté serveur depuis les sources de référence, pas de simples liens.
   Pour l'**Adaptation**, les trois sources de référence (validées par la lead Adaptation de Proparco, 2026-07) : le **[CCKP](https://climateknowledgeportal.worldbank.org/)** de la Banque mondiale (climat observé et projections, données jusqu'au niveau infranational), **[WRI Aqueduct](https://www.wri.org/applications/aqueduct/water-risk-atlas/)** (risques liés à l'eau, jusqu'au niveau bassin/local) et **[ThinkHazard!](https://thinkhazard.org/fr/)** (aléas naturels par zone). Les trois exposent une API publique exploitée par les connecteurs `src/lib/context/` ; leur granularité locale sera mobilisée quand l'ancrage local (point 2) existera.
